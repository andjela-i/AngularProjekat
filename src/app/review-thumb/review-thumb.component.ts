...
import { deleteReview, updateUser, updateUsersReviews } from 'src/store/movies.action';
...
export class ReviewThumbComponent implements OnInit{
  ...
  constructor(private store:Store<AppState>, private service:AngularprojekatService){
  ...
  ngOnInit(): void {
    ...
    if(this.review!=null){
      this.service.getUserByUnsername(this.review.username).subscribe(
        (user)=>{
  ...
  deleteReview(){
    ...
    this.store.dispatch(updateUsersReviews({user:this.reviewUser,review:this.review}))
    }
